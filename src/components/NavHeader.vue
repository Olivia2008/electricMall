        ...
        methods:{
                ...
                axios.post("/users/login",{
                  ...
                  userPwd:this.userPwd
                    ...
                    if(res.status=="0"){
                      this.errorTip = false;
                      this.loginModalFlag = false;
                      // 用户登录时调用VUEX中的nickName
                      this.$store.commit("updateUserInfo",res.result.userName);
                      // 用户登录时调用数据库中购物车中的商品数量
                      this.getCartCount();
                    }else{
                      this.errorTip = true;
                    }
            ...
            logOut(){
                axios.post("/users/logout").then((response)=>{
                    ...
                    if(res.status=="0"){
                      // this.nickName = '';
                      // $store.commit()拿到main.js中的mutations的updateUserInfo,退出后res.result.userName=“”
                        this.$store.commit("updateUserInfo",res.result.userName);
                    }
                })
            },
            getCartCount(){
              axios.get("users/getCartCount").then(res=>{
                var res = res.data;
                // 用VUEX管理购物车的商品数量，用$store.commit()拿到main.js中的mutations的updateCartCount
